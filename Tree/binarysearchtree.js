...
class BinarySearchTree {
  ...
  findSecondLargest(root) {
    ...
    while (root) {
      // Case 1: If the left child exists but there is no right child,
      // the second largest will be the largest in the left subtree.
      if (root.left && !root.right) {
        while (root.left && root.left.right) {
          root = root.left; // Move to the rightmost node of the left subtree
        }
        return root.left.value;
      }

      // Case 2: If the right child exists, and it has no children,
      // the current node is the second largest.
      if (root.right && !root.right.left && !root.right.right) {
        return root.value;
      }

      // Move to the right child to continue the search
      root = root.right;
    }
  }
  ...
  findLargest(root) {
    ...
    console.log(root.value, "largest value");
  }
...
bst.insert(3);
bst.insert(0);
console.log("get nth element", bst.getNthElement(bst.root, 1));
...
bst.inOrder(bst.root);
// console.log(bst.findSecondLargest(bst.root),"s")
